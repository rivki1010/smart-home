...
# זו רשימה של כל המכשירים שניתן לשלוט עליהם.
HA_ENTITIES = {
    "אור במשרד": "input_boolean.tvrt_mshrd", # ודא שהמזהה הזה נכון אצלך
    "דוד מים": "input_boolean.dud_mayim"   # ודא שהמזהה הזה נכון אצלך
}

# בדיקה קריטית בזמן עליית השרת
if not all([HA_URL, HA_TOKEN, GOOGLE_API_KEY]):
    print("!!! FATAL ERROR: אחד או יותר ממשתני הסביבה (HA_URL, HA_TOKEN, GOOGLE_API_KEY) לא הוגדרו בפורטל של Azure! השרת לא יעלה כראוי. !!!", flush=True)
...
if __name__ == '__main__':
    # הקוד הזה רץ רק כשאנחנו מריצים את הקובץ ישירות (למשל, במחשב המקומי)
    print(">>> מפעיל שרת פיתוח של Flask...", flush=True)
